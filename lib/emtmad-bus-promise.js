...
const debug = require('debug')('emt-api');
    ...
    return rp(options)
        .catch(function (response) {
            try {
                ...
                debug('handleAPIError->Bad request?');
            }
            if (response.statusCode && response.statusCode != 200) {
                debug(`handleAPIError->API Error 1: ${response.statusCode}`);
        ...
        .then(function (response) {
            ...
            } else if (!isNaN(returnCode) && returnCode > 1) {
                // returnCode is an error. body contains the error.
                debug(`handleAPIError->API Error 2 #${returnCode}: ${body.Description}`);
                return P.reject(Error(`API Error 2 #${returnCode}: ${body.Description}`));
            }
...
function doesStopExist(idStop) {
    ...
    if (!config.AlwaysCheckIfStopExists) {
        debug('Not checking if stop exists');
...
function getIncomingBusesToStop(idStop) {
    ...
    var formData = {
        cultureInfo: 'ES',
        idStop: idStop,
        idClient: config.EMT_ID_CLIENT,
        passKey: config.EMT_PASSKEY
    };
    var options = {
        method: 'POST',
...
function getNodesLines(stops = []) {
    ...
    // If stops is not an array throw an Error
    if (!Array.isArray(stops)) {
        let number = parseInt(stops);
        if (number > 0) {
            query += number;
        }
        else {
            throw Error('argument stops is invalid (Array||Integer)');
        }
    }
    if (stops.length > 0) {
        query = _.join(stops, '|');
    ...
    debug(`getNodesLines->Getting stop information for query: ${query.substr(0, 25)} ...`);
    var options = {
        method: 'POST',
    ...
    return handleAPIError(options)
        .then(function (body) {
            debug(`getNodesLines->Got stop information for query: ${query.substr(0, 25)} ...`);
            // if (body === false) {
            ...
            if (stops.length == 0) {
                debug(`getNodesLines->Stop not found: ${query.substr(0, 25)} ...`);
                return P.reject(Error('Stop(s) not found'));
            }
            return stops;
        });
